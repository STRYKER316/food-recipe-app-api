...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        tag_chinese = Tag.objects.create(name='Chinese', user=self.user)

        payload = {
            'title': 'Sample Chilly Chicken recipe',
            'time_minutes': 60,
            'price': Decimal('2.99'),
            'tags': [{'name': 'Chinese'}, {'name': 'Meal'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe1 = create_recipe(user=self.user, title='Sample recipe 1')
        recipe2 = create_recipe(user=self.user, title='Sample recipe 2')
        tag1 = Tag.objects.create(user=self.user, name='Chinese')
        tag2 = Tag.objects.create(user=self.user, name='Dessert')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)

        recipe3 = create_recipe(user=self.user, title='Sample recipe 3')
...
class RecipeImageUploadTests(TestCase):
    """ Tests for image upload API for recipe """
